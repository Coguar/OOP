...
BOOST_FIXTURE_TEST_SUITE(temp, TempCar);
	...
	BOOST_FIXTURE_TEST_SUITE(when_engine_turned_on, when_engine_turned_on_);
		...
		{
			BOOST_CHECK(myCar.SetGear(back));
			myCar.SetGear(neutral);
			BOOST_CHECK(myCar.SetGear(first));
		}

		BOOST_AUTO_TEST_CASE(change_gear_from_neutral_to_second_return_false)
		{
			BOOST_CHECK(!myCar.SetGear(second));
		}
	...
	{
		BOOST_CHECK(myCar.SetGear(neutral));
	}
	...
	{
		BOOST_CHECK_EQUAL(myCar.GetMovementDirection(), 1);
	}
